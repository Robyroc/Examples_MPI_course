...
int main(int argc, char** argv)
{
    ...
    if(rank == 0)
    {
        ...
        {
            for(int j = 0; j < MATRIX_LENGTH+2; j++)
                printf("%d\t", data[i][j]);
            printf("\n");
        }
    ...
    {
        MPI_Reduce(&sum, NULL, 1, MPI_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
    }
    ...
    for(int iteration = 0; iteration < 100; iteration++)
    {
        ...
        for(int i = 0; i < MATRIX_LENGTH; i++)
            data[i+1][MATRIX_LENGTH+1] = temp[1][i];

        for(int i = 0; i < MATRIX_LENGTH; i++)
            temp[0][i] = data[i+1][MATRIX_LENGTH];
        MPI_Cart_shift(topo, 1, 1, &source, &dest);
        if(iteration == 0)
            printf("%d E, sending to %d, receiving from %d\n", rank, source, dest);
        MPI_Sendrecv(temp[0], MATRIX_LENGTH, MPI_INT, dest, 3,
                    temp[1], MATRIX_LENGTH, MPI_INT, source, 3, topo, MPI_STATUS_IGNORE);
        for(int i = 0; i < MATRIX_LENGTH; i++)
            data[i+1][0] = temp[1][i];
